...
The following code chunk prepares the inputs and outputs list of the `iviRA` package to use the `pacheck` tests. In this example, only a subset of all inputs is used to limit the size of the outputs provided by `pacheck`. The functionalities of `pacheck` still work on a larger number of HE model inputs since the tests included in `pacheck` are performed on the column-level. The number of columns/ parameters included in the check therefore does not affect the working of the tests implemented in `pacheck`.
...
r_squared_rf # higher than the linear model
```

Based on the first plot where the number of trees in the random forest is plotted against the error rate, one can see that the error rate is not decreasing anymore around the 1,000 trees. Based on the variable importance plots (second plot), one can conclude that the `or_mort` and `p_rebound` model parameters have the highest impact on the results.
